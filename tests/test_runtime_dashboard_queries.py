...
class RuntimeDashboardQueriesTests(unittest.TestCase):
    ...
    def test_snapshot_reconstructs_closed_open_summary_and_diagnostics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertAlmostEqual(snapshot["summary"]["commissions"], 0.25)
            self.assertAlmostEqual(snapshot["summary"]["net_actual_pnl"], 4.75)
            self.assertEqual(snapshot["summary"]["main_pnl_source"], "executions_realized_pnl_minus_sell_commission")
            self.assertEqual(snapshot["diagnostics"]["broker_closed_trades_count"], 1)
            self.assertAlmostEqual(snapshot["diagnostics"]["broker_commissions"], 0.25)
            self.assertAlmostEqual(snapshot["diagnostics"]["broker_net_pnl"], 4.75)
            self.assertEqual(snapshot["diagnostics"]["reducer_closed_rows_count"], len(snapshot["closed_positions"]))
            self.assertIn("reducer_commissions", snapshot["diagnostics"])
            self.assertIn("reducer_net_pnl", snapshot["diagnostics"])
