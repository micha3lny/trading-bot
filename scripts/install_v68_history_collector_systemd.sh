...
ExecStart=$PYTHON_BIN -m src.live_trading.data.v68_universe_1m_parquet_collector --start-date 2026-01-01 --session-type RTH --client-id 168 --max-tasks 500 --request-sleep-seconds 0.7 --batch-size 25 --batch-sleep-seconds 10

# Hard safety timeout: collector should resume next run via checkpoint/status files.
TimeoutStartSec=3h
EOF

sudo tee "$TIMER_PATH" >/dev/null <<EOF
[Unit]
Description=Run v68 Universe 1m Parquet History Collector after US market close

[Timer]
# 20:15 UTC = 22:15 Warsaw summer time / 21:15 Warsaw winter time
