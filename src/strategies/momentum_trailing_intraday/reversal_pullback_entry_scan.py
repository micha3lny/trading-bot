...
Separates ENTRY research from EXIT research.
No exits are simulated and no orders are placed.
"""
...
def calculate_next_bar_return(session: pd.DataFrame, position: int) -> float | None:
    if position + 1 >= len(session):
        return None
    current_price = float(session.iloc[position]["close"])
    next_price = float(session.iloc[position + 1]["close"])
    if current_price == 0:
        return None
