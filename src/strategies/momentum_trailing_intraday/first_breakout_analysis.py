"""First-breakout candidate analysis for Momentum Trailing Intraday.

The raw candidate scan counts every bar that remains above opening-range high.
That can overrepresent one strong session many times. This script keeps only the
first opening-range breakout candidate per symbol/session.
...
OUTPUT_FILE = OUTPUT_DIR / "momentum_first_breakout_candidates.csv"
...
def scan_first_breakouts() -> pd.DataFrame:
...
def print_segment_analysis(df: pd.DataFrame) -> None:
    print("\nFirst breakout candidate analysis")
    ...
    segments = {
        ...
        "daily_trend_pct": [-20, -10, -5, 0, 2, 5, 10, 20],
        "entry_risk_pct": [0.5, 1.0, 1.5, 2.0, 3.0, 5.0, 8.0],
    ...
    print("\nTop 20 first breakouts by pnl_to_close_pct")
    print(
    ...
    print("\nWorst 20 first breakouts by pnl_to_close_pct")
    print(
