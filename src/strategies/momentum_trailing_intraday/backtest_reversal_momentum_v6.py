"""Reversal-momentum v6: pullback entry.

Hypothesis:
- v4 showed a narrow but real edge.
- v5 showed that broadening frequency kills net edge after costs.
- Main weakness is execution: entering immediately after confirmation often means chasing.

Logic:
1. deeply oversold daily trend
2. first opening-range breakout
3. positive next-bar confirmation
4. wait for a small pullback while price remains above opening range high
5. enter on pullback bar close
6. apply round-trip trading costs
...
def find_candidate(session: pd.DataFrame, daily_trend_pct: float) -> tuple[int, float, float, float] | None:
    ...
    for session_date, session in intraday.groupby("session_date"):
        ...
        trades.append(
            bt.simulate_exit(
                ...
                entry_risk_pct,
                daily_trend_pct,
                "reversal_momentum_v6_pullback_entry",
            )
        )
...
def main():
    ...
    gross_trades = bt.apply_position_sizing(all_trades)
    net_trades = apply_costs_to_trades(gross_trades)

    print("\nExperiment: reversal momentum v6 pullback entry (with costs)")
    print(
    ...
    bt.summarize(net_trades, regimes)
