"""Reversal pullback v19: sweet-spot MTF filters.

Research conclusion from v18/v18.1/v18.2:
- too small breakout attempts are noise
- too large breakout attempts behave more like continuation than reversal
- the best 1m triggers are strong, but not overextended

Hypothesis:
- breakout sweet spot: 1.0% - 1.8%
- 1m close strength sweet spot: 0.75 - 0.90
- ADR >= 5.0%
...
# 1m trigger sweet spot.
v17.MIN_1M_CLOSE_STRENGTH = 0.75
v17.REQUIRE_1M_CLOSE_ABOVE_PREV_CLOSE = True
v17.REQUIRE_1M_CLOSE_ABOVE_PREV_HIGH = False

# v17 has only a minimum close-strength threshold, so we patch the trigger to add
# the upper bound without duplicating the full backtest implementation.
...
def find_1m_entry_trigger_with_upper_bound(session_1m, pullback):
    ...
    for idx in range(1, len(window)):
        ...
        # Reuse v17 logic by testing a one-candidate window ending at the current row.
...
def main():
    print("\nExperiment: reversal pullback v19 sweet spot full universe (with costs)")
    print("Hypothesis:")
