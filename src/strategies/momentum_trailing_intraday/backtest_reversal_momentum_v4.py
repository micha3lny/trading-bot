"""Reversal-momentum v4.

Keeps v3 logic and changes only one thing:
- confirmation candle close_strength must be in the observed sweet spot [0.6, 0.8]

This tests whether the strongest segment from v3 analysis has real edge.
"""
...
def find_candidate(session: pd.DataFrame, daily_trend_pct: float):
    ...
    for position in range(bt.OPENING_RANGE_BARS, len(session) - 1):
        ...
        if (
            ...
            and MIN_CLOSE_STRENGTH <= confirmation_close_strength <= MAX_CLOSE_STRENGTH
        ):
            return (
                confirmation_position,
                bt.calculate_breakout_pct(confirmation_close, or_high),
                confirmation_close_strength,
...
def main():
    ...
    print("\nExperiment: reversal momentum v4")
    print(
        f"Filters: trend<= {MAX_DAILY_TREND_PCT}, breakout=[{MIN_BREAKOUT_PCT},{MAX_BREAKOUT_PCT}], "
        f"confirmation_cs=[{MIN_CLOSE_STRENGTH},{MAX_CLOSE_STRENGTH}], risk<={MAX_ENTRY_RISK_PCT}, "
        f"nret>={MIN_NEXT_BAR_RETURN_PCT}"
    )
