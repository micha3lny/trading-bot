"""Reversal pullback v14 quality-filtered.

Builds on v12, not v13.

Reasoning:
- v12 had positive expectancy after costs, but high drawdown.
- v13 reduced drawdown but killed edge because panic-only entry and time exit made wins too small.

Changes vs v12:
- keep 5m pullback entry close_strength <= 0.60
- keep original trailing/session exit from shared backtest engine
- add quality filters:
  - daily trend between -20% and -5%
  - entry risk <= 6%
...
def simulate_exit_on_5m(symbol: str, session_5m: pd.DataFrame, entry_time, breakout_pct, close_strength, entry_risk_pct, daily_trend_pct):
    ...
    return bt.simulate_exit(
        ...
        entry_risk_pct=entry_risk_pct,
        daily_trend_pct=daily_trend_pct,
        setup_type="reversal_pullback_v14_quality",
    )
...
def summarize_research(trades):
    print("\nMTF v14 quality-filtered research summary")
    print("v12-style reversal pullback, quality filters, trailing/session exit, costs included.")
...
def main():
    ...
    print("\nExperiment: reversal pullback v14 quality-filtered (with costs)")
    print(f"Loaded 5m data for {loaded_5m} symbols")
    print(
        f"15m setup: trend=[{MIN_DAILY_TREND_PCT:.2f}%, {MAX_DAILY_TREND_PCT:.2f}%], "
