...
def passes_base_v44_filter(symbol: str, day: pd.DataFrame, feats: dict[str, object], args: argparse.Namespace) -> tuple[bool, list[str], dict[str, float]]:
    reasons: list[str] = []
...
def find_continuation_entry(
    ...
    """OR breakout + hold/pullback/second-push confirmation.

    Uses only candles up to the candidate entry candle.

    Flow:
    1. find initial OR breakout
    2. wait a few minutes and reject immediate loss of OR high
    3. require controlled pullback, not a vertical chase
    4. enter on second push above post-breakout consolidation high
    """
    ...
    if pullback_pct < min_pullback_pct:
        # Too vertical / no controlled reset. This often creates chase entries.
        return None
    if pullback_pct > max_pullback_pct:
        return None

    continuation_trigger = breakout_high * (1.0 + min_second_push_pct / 100.0)
    search_start = hold_end
    max_search_end_min = max_entry_minutes_from_open

    for pos in range(search_start, len(day)):
        mins = minutes_from_open(day, pos)
        if mins > max_search_end_min:
            return None
        row = day.iloc[pos]
        # Fail fast if the setup loses OR high before confirmation.
...
def run_backtest_single_pass(args: argparse.Namespace, variants: dict[str, ExitConfig]) -> tuple[pd.DataFrame, pd.DataFrame, Counter[str], Counter[str]]:
    ...
    for symbol, day, prev_close in iter_days(Path(args.data_dir), args.recent_days):
        ...
        ok, reasons, extra = passes_base_v44_filter(symbol, day, feats, args)
        if not ok:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="v44 OR breakout continuation-confirmation backtest.")
    ...
    print("Experiment: v44 OR breakout continuation confirmation")
    print("Goal: avoid early spike-and-fade by waiting for hold/pullback/second-push confirmation.")
    ...
    out_trades = output_dir / f"v44_continuation_momentum_or_breakout_trades_{suffix}.csv"
    out_summary = output_dir / f"v44_continuation_momentum_or_breakout_summary_{suffix}.csv"
    out_rejects = output_dir / f"v44_continuation_momentum_or_breakout_rejected_{suffix}.csv"
    ...
    print("- v44 tests whether waiting for confirmation improves median/stopouts vs v42.")
    print("- If count is too low, lower --min-pullback-pct or increase --max-entry-minutes-from-open.")
    print("- If median improves but avg drops, continuation confirmation is stabilizing but cutting some large winners.")
    print("- If results are still weak, next step is premarket/catalyst filter, not more OR-only tuning.")
    return 0
