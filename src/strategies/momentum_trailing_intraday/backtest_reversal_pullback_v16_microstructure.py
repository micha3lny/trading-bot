"""Reversal pullback v16: microstructure entry.

Builds on v15/v12 idea, but improves entry quality:
- full available local universe
- volatility filter
- 15m oversold breakout attempt as setup
- 5m pullback entry only after local stabilization

Microstructure rules:
- candidate candle has weak/neutral close_strength <= 0.60
- previous 2-bar lows stop deteriorating too aggressively
- current close is above previous close OR current close is in upper half of its own range
- avoids entering on a fresh accelerating dump
...
MICRO_LOOKBACK_BARS = 2
MAX_LOW_DETERIORATION_PCT = 0.35
MIN_CURRENT_RANGE_POSITION = 0.50
...
def candle_range_position(row) -> float:
    high = float(row["high"])
    low = float(row["low"])
...
def is_microstructure_stabilizing(window: pd.DataFrame, entry_idx: int) -> bool:
    """Check last bars for local stabilization before entry."""
    if entry_idx < MICRO_LOOKBACK_BARS:
        return False

    prev_2 = window.iloc[entry_idx - 2]
    prev_1 = window.iloc[entry_idx - 1]
    current = window.iloc[entry_idx]
    ...
    low_deterioration_pct = max(0.0, (prev_low - current_low) / prev_low * 100.0)
    lows_not_accelerating = low_deterioration_pct <= MAX_LOW_DETERIORATION_PCT

    current_range_position = candle_range_position(current)
    reversal_close = current_close >= prev_close or current_range_position >= MIN_CURRENT_RANGE_POSITION

    # Avoid a clean sequence of lower low + lower close + weak close.
    accelerating_dump = (
        float(prev_2["low"]) > prev_low > current_low
        and float(prev_2["close"]) > prev_close > current_close
        and current_range_position < MIN_CURRENT_RANGE_POSITION
    )

    return lows_not_accelerating and reversal_close and not accelerating_dump
...
def find_5m_reversal_pullback_entry(session_5m: pd.DataFrame, setup: dict):
    ...
    window = after_confirmation.iloc[:PULLBACK_LOOKAHEAD_5M_BARS].reset_index(drop=True)
    ...
    for idx, row in window.iterrows():
        ...
        if not is_microstructure_stabilizing(window, idx):
            continue
...
def simulate_exit_on_5m(symbol: str, session_5m: pd.DataFrame, entry_time, breakout_pct, close_strength, entry_risk_pct, daily_trend_pct):
    ...
    return bt.simulate_exit(
        ...
        entry_risk_pct=entry_risk_pct,
        daily_trend_pct=daily_trend_pct,
        setup_type="reversal_pullback_v16_microstructure",
    )
...
def summarize_research(trades):
    print("\nMTF v16 microstructure research summary")
    print("v15 + local 5m stabilization filter, costs included.")
...
def main():
    ...
    print("\nExperiment: reversal pullback v16 microstructure full universe (with costs)")
    print(
    ...
    print(
        f"5m entry: pullback=[{MIN_PULLBACK_FROM_CONFIRMATION_PCT:.2f}%, {MAX_PULLBACK_FROM_CONFIRMATION_PCT:.2f}%], "
        f"close_strength<={MAX_5M_CLOSE_STRENGTH:.2f}, micro_low_deterioration<={MAX_LOW_DETERIORATION_PCT:.2f}%"
    )
