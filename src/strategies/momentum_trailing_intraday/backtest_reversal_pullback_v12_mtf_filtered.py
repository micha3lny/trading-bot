"""Reversal pullback v12 MTF filtered.

Builds on v11 and adds:
- 5m entry close_strength <= 0.60 filter
- pseudo-equity simulation for research-mode cash impact

This is still research mode: all valid signals are included, no portfolio ranking.
...
def simulate_exit_on_5m(symbol: str, session_5m: pd.DataFrame, entry_time, breakout_pct, close_strength, entry_risk_pct, daily_trend_pct):
    ...
    return bt.simulate_exit(
        ...
        entry_risk_pct=entry_risk_pct,
        daily_trend_pct=daily_trend_pct,
        setup_type="reversal_pullback_v12_mtf_filtered",
    )
...
def summarize_research(trades):
    print("\nMTF v12 filtered research summary")
    print("15m oversold breakout attempt, 5m weak pullback entry, costs included.")
...
def main():
    ...
    print("\nExperiment: reversal pullback v12 MTF filtered (with costs)")
