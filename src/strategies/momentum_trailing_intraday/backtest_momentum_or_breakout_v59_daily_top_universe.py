...
def build_daily_top_universe(trades: pd.DataFrame, alpha_rank: pd.DataFrame, wide_symbols: set[str], args: argparse.Namespace, top_n: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    trades = trades.copy()
    ...
    if args.expansion_before_ranking:
        trades = apply_live_safe_expansion(trades, args)
    ...
    for date, day in trades.groupby("entry_date", sort=True):
        ...
        top_symbols = set(symbol_scores.head(top_n)["symbol"].tolist())
        selected = day[day["symbol"].isin(top_symbols)].copy()
        selected_rows.append(selected)
        for rank, (_, r) in enumerate(symbol_scores.head(top_n).iterrows(), start=1):
...
def parse_top_n_values(raw: str, default_top_n: int) -> list[int]:
    if not raw.strip():
    ...
    return values or [default_top_n]
...
def main() -> int:
    ...
    parser.add_argument("--top-n-values", default="", help="Comma-separated top-N sweep, e.g. 20,30,50,100")
    parser.add_argument("--alpha-weight", type=float, default=0.25)
    parser.add_argument("--apply-live-safe-expansion", action="store_true")
    parser.add_argument("--expansion-before-ranking", action="store_true")
    ...
    top_n_values = parse_top_n_values(args.top_n_values, args.top_n)

    print("Experiment: v59 daily top-N universe")
    print(f"Trades CSV: {args.trades_csv}")
    print(f"Alpha rank: {args.alpha_rank_csv}")
    print(f"Top N values/day: {top_n_values}")
    print(f"Live-safe expansion after ranking: {args.apply_live_safe_expansion}")
    print(f"Expansion before ranking: {args.expansion_before_ranking}")
    ...
    wide_trades_raw = trades[trades["symbol"].isin(wide)].copy()
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    all_summaries = []
    for top_n in top_n_values:
        selected_trades, daily_universe = build_daily_top_universe(trades, alpha, wide, args, top_n)
        wide_trades = wide_trades_raw.copy()

        if args.apply_live_safe_expansion and not args.expansion_before_ranking:
            wide_trades = apply_live_safe_expansion(wide_trades, args)
            selected_trades = apply_live_safe_expansion(selected_trades, args)
        elif args.expansion_before_ranking:
            wide_trades = apply_live_safe_expansion(wide_trades, args)

        suffix = f"top_{top_n}"
        if args.expansion_before_ranking:
            suffix += "_expansion_before_rank"
        if args.apply_live_safe_expansion:
            suffix += "_live_safe_expansion"

        baseline_label = "v59_baseline_wide_all_candidates"
        top_label = f"v59_daily_top_{top_n}"
        if args.expansion_before_ranking:
            baseline_label += "_expansion_before_rank"
            top_label += "_expansion_before_rank"
        if args.apply_live_safe_expansion:
            baseline_label += "_live_safe_expansion"
            top_label += "_live_safe_expansion"

        baseline_summary, baseline_costed, baseline_rejected = run_case(baseline_label, wide_trades, args)
        top_summary, top_costed, top_rejected = run_case(top_label, selected_trades, args)

        baseline_summary = add_selection_metadata(baseline_summary, 0, wide_trades)
        top_summary = add_selection_metadata(top_summary, top_n, selected_trades)
        ...
        print(f"\n=== v59 daily top-N comparison: top {top_n} ===")
    ...
    final_path = output_dir / "v59_daily_topn_sweep_summary.csv"
    final_summary.to_csv(final_path, index=False)

    print("\n=== v59 top-N sweep summary ===")
    print(final_summary.to_string(index=False, float_format=lambda x: f"{x:.2f}"))
    print(f"\nSaved sweep summary: {final_path}")

    print("\nInterpretation hints:")
    print("- expansion_before_ranking tests the preferred live architecture: broad universe -> expansion filter -> ranking -> top N -> portfolio.")
    print("- apply_live_safe_expansion without expansion_before_ranking tests the older order: ranking -> expansion filter.")
    print("- Top-N sweep checks whether top20/top30/top50/top100 is enough after the expansion filter.")
    return 0
