"""Reversal-momentum v8 MTF: 15m setup, 5m execution.

Purpose:
- detect the setup on 15-minute bars
- execute entry on 5-minute bars using a pullback after confirmation
- apply trading costs
...
# 15m setup filters, based on v4/v5 research.
...
PULLBACK_LOOKAHEAD_5M_BARS = 9  # 45 minutes after 15m confirmation
...
def calculate_next_bar_return(session: pd.DataFrame, position: int) -> float | None:
    ...
    return calculate_return_pct(current_close, next_close)


def find_15m_setup(session_15m: pd.DataFrame, daily_trend_pct: float):
    """Return setup details from 15m bars, or None."""
    ...
    for breakout_position in range(bt.OPENING_RANGE_BARS, len(session_15m) - 1):
        ...
        if (
            ...
            return {
                ...
                "confirmation_close_strength": confirmation_close_strength,
                "daily_trend_pct": daily_trend_pct,
            }

        # first breakout only for symbol/session
        return None
...
def find_5m_pullback_entry(session_5m: pd.DataFrame, setup: dict):
    """Find 5m pullback entry after 15m confirmation."""
    ...
    for entry_position, row in window.iterrows():
...
def simulate_exit_on_5m(symbol: str, session_5m: pd.DataFrame, entry_time, breakout_pct, close_strength, entry_risk_pct, daily_trend_pct):
    ...
    return bt.simulate_exit(
        symbol=symbol,
        session=session_5m.reset_index(drop=True),
        entry_position=entry_position,
        breakout_pct=breakout_pct,
        close_strength=close_strength,
        entry_risk_pct=entry_risk_pct,
        daily_trend_pct=daily_trend_pct,
        setup_type="reversal_momentum_v8_mtf_5m_entry",
    )
...
def summarize_research(trades):
    print("\nMTF research-mode signal summary")
    print("15m setup, 5m pullback entry, costs included.")
...
def main():
    ...
    print("\nExperiment: reversal momentum v8 MTF (15m setup, 5m entry, with costs)")
