"""Reversal-momentum v9 MTF relaxed.

Builds on v8:
- keep 15m setup + 5m pullback execution
- keep trading costs
- relax setup filters to increase signal frequency
...
def simulate_exit_on_5m(symbol: str, session_5m: pd.DataFrame, entry_time, breakout_pct, close_strength, entry_risk_pct, daily_trend_pct):
    ...
    return bt.simulate_exit(
        ...
        entry_risk_pct=entry_risk_pct,
        daily_trend_pct=daily_trend_pct,
        setup_type="reversal_momentum_v9_mtf_relaxed",
    )
...
def summarize_research(trades):
    print("\nMTF relaxed research-mode signal summary")
    print("15m relaxed setup, 5m pullback entry, costs included.")
...
def main():
    ...
    print("\nExperiment: reversal momentum v9 MTF relaxed (15m setup, 5m entry, with costs)")
