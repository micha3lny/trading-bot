"""Reversal pullback v13: panic entry + time exit.

Builds on v12 best-known configuration.

Changes:
- 5m entry candle close_strength <= 0.35 (panic / flush only)
- entry breakout_pct >= 1.00 at pullback entry
- custom exit for mean reversion:
  - wider initial stop: 2.00%
  - time exit after 15 bars (75 minutes on 5m)
  - optional profit cap/take-profit: 3.00%
- costs included
- pseudo-equity included
...
MAX_5M_CLOSE_STRENGTH = 0.35
MIN_ENTRY_BREAKOUT_PCT = 1.00
MAX_5M_ENTRY_RISK_PCT = 10.0
MAX_CLOSE_BELOW_OR_HIGH_PCT = 0.75

INITIAL_STOP_LOSS_PCT = 2.00
...
def find_5m_reversal_pullback_entry(session_5m: pd.DataFrame, setup: dict):
    ...
    for _, row in window.iterrows():
        ...
        if (
            ...
            and breakout_pct >= MIN_ENTRY_BREAKOUT_PCT
...
def simulate_time_exit(symbol: str, session_5m: pd.DataFrame, entry_time, breakout_pct, close_strength, entry_risk_pct, daily_trend_pct):
    ...
    entry_bar = session_5m.iloc[entry_position]
    entry_price = float(entry_bar["close"])
    entry_time_str = str(entry_bar["date"])
    session_date = str(entry_bar["date"].date())

    bars_after_entry = session_5m.iloc[entry_position + 1 : entry_position + 1 + TIME_EXIT_BARS]
    if bars_after_entry.empty:
        return bt.BacktestTrade(
            ...
            entry_risk_pct,
            daily_trend_pct,
            "reversal_pullback_v13_panic_time_exit",
        )

    stop_price = entry_price * (1.0 - INITIAL_STOP_LOSS_PCT / 100.0)
...
def summarize_research(trades):
    print("\nMTF v13 panic/time-exit research summary")
    print("15m oversold breakout attempt, 5m panic pullback entry, time exit, costs included.")
...
def main():
    ...
    print("\nExperiment: reversal pullback v13 panic/time exit (with costs)")
    ...
    print(
        f"5m panic entry: lookahead={PULLBACK_LOOKAHEAD_5M_BARS}, "
        f"pullback=[{MIN_PULLBACK_FROM_CONFIRMATION_PCT:.2f}%, {MAX_PULLBACK_FROM_CONFIRMATION_PCT:.2f}%], "
        f"close_strength=[{MIN_5M_CLOSE_STRENGTH:.2f}, {MAX_5M_CLOSE_STRENGTH:.2f}], "
        f"entry_breakout>={MIN_ENTRY_BREAKOUT_PCT:.2f}%, "
        f"max_close_below_or_high={MAX_CLOSE_BELOW_OR_HIGH_PCT:.2f}%"
    )
    print(
        f"Exit: stop={INITIAL_STOP_LOSS_PCT:.2f}%, take_profit={TAKE_PROFIT_PCT:.2f}%, "
        f"time_exit_bars={TIME_EXIT_BARS}"
    )
