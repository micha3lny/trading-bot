"""Reversal pullback v11 MTF.

This is a pivot away from chasing breakout momentum.

Hypothesis from v9/v10:
- Stronger breakouts after oversold conditions tend to underperform.
- The better edge is likely a reversal/mean-reversion entry after the first breakout attempt.

Logic:
1. 15m detects oversold context and first opening-range breakout attempt.
2. 5m waits for a deeper pullback after confirmation.
3. Entry is allowed BELOW opening range high if structure is not completely broken.
4. Exit reuses the existing intraday stop/trailing logic.
5. Costs are applied.
...
# 15m setup: oversold + first breakout attempt, but less strict than v4.
...
# 5m mean-reversion pullback entry.
PULLBACK_LOOKAHEAD_5M_BARS = 12  # 60 minutes after confirmation
MIN_PULLBACK_FROM_CONFIRMATION_PCT = 0.30
MAX_PULLBACK_FROM_CONFIRMATION_PCT = 2.50
MIN_5M_CLOSE_STRENGTH = 0.40
MAX_5M_ENTRY_RISK_PCT = 10.0

# Do not require close above opening-range high anymore.
# But avoid total breakdowns: entry close cannot be too far below OR high.
...
def calculate_next_bar_return(session: pd.DataFrame, position: int) -> float | None:
    if position + 1 >= len(session):
        return None
    return calculate_return_pct(
        float(session.iloc[position]["close"]),
        float(session.iloc[position + 1]["close"]),
    )
...
def find_15m_setup(session_15m: pd.DataFrame, daily_trend_pct: float):
    ...
    for breakout_position in range(bt.OPENING_RANGE_BARS, len(session_15m) - 1):
        ...
        # first breakout attempt only for symbol/session
        return None
...
def simulate_exit_on_5m(symbol: str, session_5m: pd.DataFrame, entry_time, breakout_pct, close_strength, entry_risk_pct, daily_trend_pct):
    ...
    return bt.simulate_exit(
        ...
        entry_risk_pct=entry_risk_pct,
        daily_trend_pct=daily_trend_pct,
        setup_type="reversal_pullback_v11_mtf",
    )
...
def summarize_research(trades):
    print("\nMTF v11 reversal-pullback research summary")
    print("15m oversold breakout attempt, 5m deeper pullback entry, costs included.")
...
def main():
    ...
    print("\nExperiment: reversal pullback v11 MTF (with costs)")
    print(f"Loaded 5m data for {loaded_5m} symbols")
    print(
        f"15m setup: trend<= {MAX_DAILY_TREND_PCT:.2f}%, "
        f"breakout_attempt=[{MIN_BREAKOUT_PCT:.2f}%, {MAX_BREAKOUT_PCT:.2f}%], "
    ...
    print(
        f"5m pullback entry: lookahead={PULLBACK_LOOKAHEAD_5M_BARS}, "
        f"pullback=[{MIN_PULLBACK_FROM_CONFIRMATION_PCT:.2f}%, {MAX_PULLBACK_FROM_CONFIRMATION_PCT:.2f}%], "
        f"close_strength>={MIN_5M_CLOSE_STRENGTH:.2f}, "
        f"max_close_below_or_high={MAX_CLOSE_BELOW_OR_HIGH_PCT:.2f}%, "
        f"entry_risk<={MAX_5M_ENTRY_RISK_PCT:.2f}%"
    )
